...
/** 获取购物车mock数据 */
function mockFetchCartGroupData(params: any) {
...
/** 获取购物车数据 */
export function fetchCartGroupData(params?: any) {
  if (config.useMock) {
    return mockFetchCartGroupData(params);
  }
  return request({ url: '/api/v1/cart', method: 'GET', needAuth: true }).then((data: any = {}) => {
    return { data: attachCartSummary(data) };
  });
}

/** 添加商品到购物车 */
export function addCartItem({ skuId, quantity }: { skuId: string | number; quantity: number }) {
  if (config.useMock) {
    const { delay } = require('../_utils/delay');
  ...
  return request({
    ...
    needAuth: true,
  }).then((data: any = {}) => ({ data: attachCartSummary(data) }));
}

/** 更新购物车商品数量 */
export function updateCartItem(skuId: string | number, { quantity }: { quantity: number }) {
  if (config.useMock) {
    const { delay } = require('../_utils/delay');
...
/** 删除购物车商品 */
export function deleteCartItem(skuId: string | number) {
  if (config.useMock) {
    const { delay } = require('../_utils/delay');
    return delay(100).then(() => ({ data: {} }));
  }
...
export function attachCartSummary(cart: any) {
  ...
  stores.forEach((store: any) => {
    (store.promotionGoodsList || []).forEach((promotion: any) => {
      (promotion.goodsPromotionList || []).forEach((item: any) => goods.push(item));
    });
  });
